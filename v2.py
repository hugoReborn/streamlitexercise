...
@st.cache(
    persist=True)  # decoramos la funcion con cache persist para que no haga la computacion de datos cada vez que se realice algun cambio o filtro
def load_data(nrows):  # funcion especifica la cantidad de rows que entregara
    data = pd.read_csv(DATA_URL, nrows=nrows, parse_dates=[['CRASH_DATE',
                                                            'CRASH_TIME']])  # almacenaremos los datos en nuestra variable data y leeremos este archivo con pandas read
    data.dropna(subset=['LATITUDE', 'LONGITUDE'],
                inplace=True)  # el primer argunmento sera el path del archivo o la url de donde estamos sacando los datos
    lowercase = lambda x: str(
        x).lower()  # los datos tiene unas columnas , que es la fecha del choque y la hora del choque
    data.rename(lowercase, axis='columns',
                inplace=True)  # usamos un parse dates para convertirlas en formato datetime indicamos las columnas que pasaremos al parse dates
    data.rename(columns={'crash_date_crash_time': 'date/time'},
                inplace=True)  # tomamos los na datos  almacenados en el data
    return data
